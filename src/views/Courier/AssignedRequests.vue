    ...
    <div class="overflow-x-auto">
      <table class="w-full table-auto">
        ...
        <tbody class="bg-tabs divide-y divide-gray-200">
          ...
          <tr v-for="request, index in filteredRequests" :key="request.id" class="transition-colors text-center">
        ...
        <div class="relative p-4 w-full max-w-md" @click.stop>
          <div class="relative bg-tabs rounded-lg shadow">
            <button type="button" @click="closeScanner"
              class="absolute top-3 right-2.5 text-section bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white">
            ...
            <div class="p-4 md:p-5 text-center">
              <h3 class="text-xl font-semibold text-primary mb-2">{{ $t('common.scanQrToCollect') }}</h3>
              <p class="text-sm text-section mb-2">
                Request: <span class="font-bold">{{ activeRequestNumber }}</span>
              </p>
              <p class="text-section mb-4">{{ infoText }}</p>
              ...
              <div class="flex justify-center gap-3">
                <button @click="closeScanner" type="button"
                  class="py-2.5 px-5 text-sm font-medium text-section focus:outline-none bg-tabs rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-[#2C702C]">
          ...
          <div class="relative bg-tabs rounded-lg shadow">
            ...
            <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
              ...
              <button type="button" @click="showDetailsModal = false"
                class="text-section bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center">
                <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none"
                  viewBox="0 0 14 14">
                  <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
            ...
            <div class="p-4 md:p-5">
              <div class="grid gap-4 mb-4 sm:grid-cols-2">
                <div>
                  <h4 class="text-sm font-medium text-section">{{ $t('common.fullName') }}</h4>
                ...
                <div>
                  <h4 class="text-sm font-medium text-section">{{ $t('common.orderStatus') }}</h4>
              ...
              <!-- Waste Items Section -->
              <div class="mb-4">
                <h4 class="text-lg font-semibold text-primary mb-3 border-b pb-2">{{ $t('common.wasteItemsToCollect')
                  }}</h4>
                <div class="space-y-3 max-h-64 overflow-y-auto">
                  <div v-for="item in details.items" :key="item.id"
                    ...
                    <img :src="item.item?.image_url" :alt="item.item?.name"
                      class="w-16 h-16 object-cover rounded-lg bg-tabs border border-gray-200" />

                    <!-- Item Details -->
                    <div class="flex-1">
                      <h5 class="font-semibold text-primary">{{ item.item?.name }}</h5>
              ...
              <!-- Total Points -->
              <div class="bg-myprofile rounded-lg p-4 mb-4">
                <div class="flex justify-between items-center">
                  <h4 class="text-sm font-medium text-section">{{ $t('common.totalPoints') }}</h4>
              ...
              <div class="flex justify-end">
                <button @click="showDetailsModal = false" type="button"
                  class="py-2.5 px-5 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-primary">
                  {{ $t('common.close') }}
                </button>
              </div>
            </div>
