<template>
  ...
  <div class="relative px-4 sm:px-6 md:px-8 pb-2 sm:pb-12    ">
    ...
    <div class="relative flex flex-col sm:flex-row items-center justify-center text-center lg:text-left gap-6 md:gap-8 px-4 sm:px-6 md:px-10 pt-16 sm:pt-18  md:pt-22 lg:pt-28
      ...
      <!-- Logo -->
      <img src="/logos/K.png" alt="Karakib Logo" class="w-28 sm:w-32 md:w-36 lg:w-40 xl:w-44 drop-shadow-lg shrink-0" />
  ...
  <!-- links -->
  <div class="px-4 sm:px-6 md:px-12 lg:px-16 relative z-20 flex flex-wrap gap-3 md:gap-4 -mt-5 mb-2 md:mb-4">
    ...
    <div v-for="(product, index) in filteredProducts" :key="index"
      ...
      <router-link :to="{ name: 'product-desc', params: { id: product.id } }" class="block rounded-t-xl">
        ...
        <div class="px-3 pt-3 pb-2">
          ...
          <div class="flex">
            <svg v-for="i in 5" :key="i" class="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5"
              :class="i <= Math.floor(productRating(product)) ? 'text-yellow-400' : 'text-gray-300'" fill="currentColor"
              viewBox="0 0 20 20">
              <path
...
export default {
  ...
  methods: {
    ...
    productRating(product) {
      return Number(product.average_rating).toFixed(1)
    },
  },
  mounted() {
